...
if (typeof(extensions.nav) == 'undefined') {
	...
	extensions.nav = function() {
		...
		this.init = function() {
			...
			var osVersion = String(Components.classes["@mozilla.org/xre/app-info;1"].getService(Components.interfaces.nsIXULRuntime).OS).toLowerCase();
		...
		this.onPopupShowing = function(aEvent) {
			...
			if (!this.fileExists(aFileData)) this.fileWrite(aFileData, '[]');
			...
			for (var id in bookmarks) {
				if (bookmarks[id] != '') this.appendItem(bookmarks[id], anElement, false);
			}
			...
			try {
				...
				while (drivesEnum.hasMoreElements()) {
					drives[drives.length] = drivesEnum.getNext().QueryInterface(Components.interfaces.nsILocalFile).path
				}
			} catch (e) {
				var drives = [];
			}
		...
		this.appendItem = function(aURL, aPreviousNode, aTemporalNode) {
			...
			if (this.fileIsFolder(aURL)) add.setAttribute("class", 'menuitem-iconic nav-d');
			else {
			...
			if (aTemporalNode) add.setAttribute("temporal", true);
		...
		this.browserNavigate = function(aEvent) {
			...
			if ((tagName == 'xul:menu' || tagName == 'menu' || tagName == 'menuitem') && item.hasAttribute('value')) {
				if (aEvent.type == 'mouseover') item.setAttribute('isFocused', true);
				else if (aEvent.type == 'mouseout') item.removeAttribute('isFocused');
				...
				if (!item.hasAttribute('done')) {
					...
					if (aEvent.type == 'mouseover' && !item.hasAttribute('retrieving')) {
						...
						item.interval = setTimeout(function() {
							extensions.nav.browserNavigateRequestCategories(item);
						}, 190);
		...
		this.browserNavigateBuildSubMenu = function(item, aCategories) {
			...
			for (var id in aCategories) {
				...
				if (this.fileIsFolder(aCategories[id])) add.setAttribute('class', 'menuitem-iconic nav-d');
				else {
					add.setAttribute('done', 'true');
					add.setAttribute('class', 'menuitem-iconic nav-f');
			...
			if (item && item.hasAttribute('isFocused')) {
				if (item && item.parentNode) {
					...
					if (item.hasAttribute('temporal')) menu.setAttribute('temporal', item.getAttribute('temporal'));
		...
		this.browserClick = function(aEvent) {
			...
			if (!item.hasAttribute('value')) {
				...
				if (aEvent.button == 2) this.stopEvent(aEvent);
				...
				if (item.hasAttribute('action')) {
					...
					if (action == 'bookmark_active_tab') this.bookmarkAdd(this.filePathFromFileURI(this.documentFocusedGetLocation()));
		...
		this.contextPopupShowing = function(aEvent) {
			setTimeout(function() {
				extensions.nav.getBrowserElement('places-files-popup').hidePopup();
			}, 0);
			//if the root menu was right clicked
			if (this.selectedItem && (
			this.selectedItem.parentNode == this.getElement('menupopup') || this.selectedItem.parentNode.parentNode == this.getElement('menupopup'))) {
				var childNodes = aEvent.currentTarget.childNodes;
				for (var i = 0; i < childNodes.length; i++) {
					var item = childNodes[i];
					if (item.hasAttribute('action')) {
						if (
						item.getAttribute('action') == 'bookmark_add' || item.getAttribute('action') == 'bookmark_add_open') {
			...
			try {
				...
				if (aDirectory.isDirectory()) {
					...
					while (entries.hasMoreElements()) {
						...
						if (entry.isDirectory()) folderContentD[folderContentD.length] = path;
						else folderContentF[folderContentF.length] = path;
					}
		...
		this.extensionDirectory = function() {
			var extensionDirectory = Components.classes["@mozilla.org/file/directory_service;1"].getService(Components.interfaces.nsIProperties).get("ProfD", Components.interfaces.nsIFile);
		...
		this.categoryAbbreviate = function(aURL) {
			...
			if (this.subStrCount(aURL, '/') > 5) return aURL.replace(/\/+$/, '').replace(/.*\/([^\/]+\/[^\/]+\/[^\/]+\/[^\/]+)$/, "…$1");
			else return aURL;
		}
		...
		this.removeChilds = function(anElement) {
			if (anElement.hasChildNodes()) {
				...
				for (var a = 0; a < length; a++) {
					if (!anElement.childNodes[a].hasAttribute('locked')) deletion[deletion.length] = anElement.childNodes[a];
				}
		...
		this.placesPopupShown = function(event) {
			if (event.currentTarget == event.originalTarget) {
				if (ko.places.manager._clickedOnRoot()) this.rootButton = true;
				else this.rootButton = false;
			}
			return true;
		}
		...
		this.getPlacesPath = function() {
			if (ko.places && ko.places.manager && ko.places.manager.currentPlace && ko.places.manager.currentPlace != '') return String(ko.places.manager.currentPlace);
			else return '';
		}
		this.getSelectedPathFolder = function(event) {
			var selected = this.getSelectedPaths(event)[0];
			if (this.fileIsFolder(selected)) {} else selected = this.fileDirname(selected);
			return selected;
		}
		...
		this.openURL = function(aFilePath, newTab) {
			if (this.fileIsFolder(aFilePath)) //folders
			{
				...
				if (newTab) ko.open.multipleURIs([aFilePath]);
			}
		}
		...
		this.documentGetFocused = function() {
			...
			if (aDoc) return aDoc;
			else this.error('no document focused');
		}
		this.documentGetLocation = function(aDocument) {
			if (aDocument.displayPath) return aDocument.displayPath;
			else this.error('document has no location');
		}
		this.documentGetFromTab = function(aTab) {
			if (aTab.document) return aTab.document;
			else this.error('tab has no document');
		}
		this.tabGetFocused = function() {
			if (ko.views.manager.currentView) return ko.views.manager.currentView;
			else this.error('no tab focused');
		}
		...
		this.fileIsFolder = function(aFilePath) {
			try {
				...
				if (aFile.exists() && aFile.isDirectory()) return true;
				else return false;
			} catch (e) {
				return false;
			}
		...
		this.fileDirname = function(aFilePath) {
			var aDestination = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
		...
		this.filePathFromFileURI = function(aURI) {
			try {
				if (!this.ios) this.ios = Components.classes["@mozilla.org/network/io-service;1"].
				getService(Components.interfaces.nsIIOService);

				return String(this.ios.newURI(aURI, null, null).QueryInterface(Components.interfaces.nsIFileURL).file.path);
			} catch (e) {
				return aURI;
			}
		}
		...
		this.inArray = function(anArray, some) {
			for (var id in anArray) {
				if (anArray[id] == some) return true;
			}
			return false;
		}
